...
/// with its display name and icon. NSIS installs already stamp the AUMID onto
/// their shortcut, but Windows still needs these assets for the toast header;
/// portable zips have no shortcut at all. Idempotent, per-user, no admin.
#[cfg(target_os = "windows")]
pub(crate) fn init(app: &tauri::AppHandle) {
    ...
    if let Err(error) = register_aumid(&app_id, "Carrier", icon_path.as_deref()) {
...
#[cfg(target_os = "windows")]
fn register_aumid(aumid: &str, display_name: &str, icon_path: Option<&Path>) -> Result<(), String> {
